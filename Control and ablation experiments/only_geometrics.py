...
class GeometryScalarTokenResidualNet(nn.Module):
    ...
    Why this version:
        ...
        uses global mean pooling over the four geometry tokens.
    ...
    def forward(self, geom):
        ...
        # Global mean pooling over the four geometry tokens.
        pooled = z.mean(dim=1)
...
def run_epoch(model, loader, loss_fn, res_mean, res_scale, device, optimizer=None):
    ...
    total = 0.0
    for batch in loader:
        geom, target, smm, *_ = unpack(batch, device)
...
def train_model(model, train_loader, val_loader, res_scaler, device):
    ...
    history = []
    t0 = time.time()
    print("\n开始训练：Geometry scalar-token mean-pooling residual ablation...\n")
...
def build_summary(df, train_idx, val_idx, test_idx, split_info, history, best_epoch, best_val, metrics):
    return {
        ...
        "model_config": {"MODEL": "GeometryScalarTokenResidualNet", "INPUT_TOKENS": "[Lambda/lambda, f, h/lambda, theta/90]",
                         "REMOVED_TOKENS": "Mode0, Mode1, CLS", "TOKEN_AGGREGATION": "mean_pooling",
