...
OUTPUT_DIR = ROOT / "direct_predict_results"
MODEL_DIR = OUTPUT_DIR / "models"
EXPERIMENT_NAME = "token_direct_rcwa_10000"
...
def fit_transform_features(x: dict, train_idx: np.ndarray):
    ...
    scalers["rcwa"].fit(x["rcwa"][train_idx])
    xs = {
        ...
        "target_scaled": scalers["rcwa"].transform(x["rcwa"]).astype(np.float32),
    }
    return xs, scalers
...
class ModalTokenDirectRCWANet(nn.Module):
    """
    Input tokens: [Geometry, Mode0, Mode1]
    Token aggregation: mean pooling over the three Transformer output tokens.
    Output: scaled [eta0_RCWA, etam1_RCWA]
    No SMM addition is used in final prediction.
    """
    def __init__(self):
        super().__init__()
...
def inverse_scale_efficiency_torch(pred_scaled, target_mean, target_scale):
    """使用训练集 StandardScaler 参数，以可微方式还原网络预测的物理效率。"""
...
def run_epoch(model, loader, loss_fn, target_mean, target_scale, device, optimizer=None):
    ...
    for batch in loader:
        ...
        with torch.set_grad_enabled(is_train):
            ...
            # 数据损失：标准化 RCWA 效率空间中的 SmoothL1。
            data_loss = loss_fn(pred, target)

            # 物理约束：作用于裁剪前、反标准化后的直接预测效率。
            pred_eff_raw = inverse_scale_efficiency_torch(pred, target_mean, target_scale)
...
def train_model(model, train_loader, val_loader, target_scaler, device):
    loss_fn = nn.SmoothL1Loss(beta=SMOOTH_L1_BETA)
    target_mean = torch.as_tensor(target_scaler.mean_, dtype=torch.float32, device=device)
    target_scale = torch.as_tensor(target_scaler.scale_, dtype=torch.float32, device=device)
    ...
    history = []
    t0 = time.time()
    print("\n开始训练：Token Transformer direct RCWA...\n")
    for epoch in range(1, MAX_EPOCHS + 1):
        train_loss = run_epoch(model, train_loader, loss_fn, target_mean, target_scale, device, optimizer)
        val_loss = run_epoch(model, val_loader, loss_fn, target_mean, target_scale, device)
...
@torch.no_grad()
def predict(model, loader, target_scaler, device):
    model.eval()
    ...
    pred_eff_raw = target_scaler.inverse_transform(np.vstack(pred_s))
    ...
    return {
        ...
        "smm": smm,
        "rcwa": rcwa,
        "true_res": np.vstack(res_all),
        "implied_pred_res": pred_eff_raw - smm,
...
def save_prediction_csv(df, test):
    ...
    rcwa, nn_pred = test["rcwa"], test["pred_eff"]
    ...
    pred_df["implied_pred_res_eta0_TE"] = test["implied_pred_res"][:, 0]
    pred_df["implied_pred_res_etam1_TE"] = test["implied_pred_res"][:, 1]
    ...
    pred_df.to_csv(OUTPUT_DIR / "test_predictions_direct_rcwa_10000.csv", index=False)
...
def build_summary(df, train_idx, val_idx, test_idx, split_info, history, best_epoch, best_val, metrics):
    return {
        ...
        "learning_target": "direct_scaled_RCWA_efficiency",
        "final_prediction_form": "eta_pred = NN(geometry, mode0, mode1)",
        "use_smm_as_network_input": False,
        "use_smm_final_addition": False,
        "model_config": {"D_MODEL": D_MODEL, "N_HEADS": N_HEADS, "N_LAYERS": N_LAYERS,
                         "DROPOUT": DROPOUT, "MLP_HIDDEN": MLP_HIDDEN,
                         "INPUT_TOKENS": "[Geometry, Mode0, Mode1]",
                         "USE_CLS_TOKEN": False, "TOKEN_AGGREGATION": "mean_pooling"},
...
def save_model_and_scalers(model, scalers, split_info):
    torch.save({
        ...
        "split_info": split_info,
    }, MODEL_DIR / "best_token_direct_rcwa_net_10000.pth")
    joblib.dump(scalers, MODEL_DIR / "scalers_direct_rcwa_10000.pkl")
...
def main():
    ...
    print("=" * 80)
    print("Token Transformer direct RCWA prediction - 10000 samples")
    print("Device:", device)
    ...
    np.savez(OUTPUT_DIR / "split_indices_direct_rcwa_10000.npz",
    ...
    model = ModalTokenDirectRCWANet().to(device)
    print("\n模型参数量:", sum(p.numel() for p in model.parameters()))
    history, best_epoch, best_val = train_model(model, train_loader, val_loader, scalers["rcwa"], device)
    pd.DataFrame(history).to_csv(OUTPUT_DIR / "training_history_direct_rcwa_10000.csv", index=False)
    test = predict(model, test_loader, scalers["rcwa"], device)
    ...
    metrics = {
        "SMM_overall": smm_m,
        "Direct_NN_overall": nn_m,
        "Direct_NN_eta0_efficiency": metric_dict(rcwa_true[:, 0], nn_pred[:, 0]),
        "Direct_NN_etam1_efficiency": metric_dict(rcwa_true[:, 1], nn_pred[:, 1]),
        ...
        "Direct_NN_raw_unclipped_overall": metric_dict(rcwa_true, test["pred_eff_raw"]),
    }
    save_json(build_summary(df, train_idx, val_idx, test_idx, split_info, history, best_epoch, best_val, metrics),
              OUTPUT_DIR / "summary_metrics_direct_rcwa_10000.json")
    ...
    print(f"Direct NN MAE:  {nn_m['MAE']:.6f}")
    print(f"MAE 降低比例:  {improve:.2f}%")
    print(f"Direct NN R2:   {nn_m['R2']:.6f}")
    print(f"Direct NN MaxError: {nn_m['MaxAbsError']:.6f}")
