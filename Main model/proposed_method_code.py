"""
主要作用：利用 SMM 引导的模态 Token 残差学习预测 TE 偏振利特罗光栅的 0 级和 -1 级衍射效率。
网络架构：输入序列为 [CLS, Geometry, Mode0, Mode1]；各 Token 映射至 96 维空间，经 2 层、
2 头 Transformer 编码器后，将 CLS 特征输入 96-64-2 的 MLP，预测 RCWA-SMM 残差，最终输出
eta_pred = eta_SMM + residual_NN。网络不使用 SMM 效率、模态差分或 overlap 积分作为输入。
训练策略：10000 个样本按 8:1:1 划分，随机种子 42；仅用训练集拟合标准化器；总损失由
SmoothL1 数据损失和最终效率的物理约束损失组成；采用 AdamW（lr=3e-4，weight_decay=1e-4）、
batch size 256、最多 1000 轮、梯度裁剪 1.0；
ReduceLROnPlateau 连续 20 轮未改善时将学习率减半，最低 1e-6；连续 100 轮未改善则早停。
输出内容：模型、标准化器、数据划分、训练历史、测试集预测与评价指标；仅本脚本生成论文图片，
图片采用白色背景、Times New Roman 字体、900 dpi，并只保存 PNG 和 SVG 格式。
"""
...
# 建议在 0.01、0.1、1.0 中做验证集对比；先使用 0.1 作为较弱的物理约束。
...
def physics_constraint_stats(pred_eff_raw: np.ndarray, tol: float = 1e-12) -> dict:
    """统计裁剪前预测对三项物理约束的违反情况。"""
    pred_eff_raw = np.asarray(pred_eff_raw)
    negative_eta0 = np.maximum(-pred_eff_raw[:, 0], 0.0)
    negative_etam1 = np.maximum(-pred_eff_raw[:, 1], 0.0)
    energy_excess = np.maximum(pred_eff_raw.sum(axis=1) - 1.0, 0.0)
    any_violation = (negative_eta0 > tol) | (negative_etam1 > tol) | (energy_excess > tol)
    return {
        "physics_loss_mean": float(np.mean(
            negative_eta0 ** 2 + negative_etam1 ** 2 + energy_excess ** 2
        )),
        "any_violation_rate_percent": float(np.mean(any_violation) * 100.0),
        "negative_eta0_rate_percent": float(np.mean(negative_eta0 > tol) * 100.0),
        "negative_etam1_rate_percent": float(np.mean(negative_etam1 > tol) * 100.0),
        "energy_sum_above_one_rate_percent": float(np.mean(energy_excess > tol) * 100.0),
        "max_negative_eta0_magnitude": float(np.max(negative_eta0)),
        "max_negative_etam1_magnitude": float(np.max(negative_etam1)),
        "max_energy_excess": float(np.max(energy_excess)),
    }
...
def run_epoch(model, loader, data_loss_fn, res_mean, res_scale, device, optimizer=None):
    is_train = optimizer is not None
    model.train(is_train)
    totals = {"total": 0.0, "data": 0.0, "physics": 0.0}

    for batch in loader:
        mode, geom, target, smm, *_ = unpack(batch, device)
        with torch.set_grad_enabled(is_train):
            pred_res_scaled = model(mode, geom)

            # 数据项仍在标准化残差空间计算，保持原残差学习框架不变。
            data_loss = data_loss_fn(pred_res_scaled, target)

            # 物理项必须作用于裁剪前的最终效率，而不是标准化残差。
            pred_res = inverse_scale_residual_torch(pred_res_scaled, res_mean, res_scale)
        ...
        totals["data"] += data_loss.item() * batch_size
        totals["physics"] += physics_loss.item() * batch_size
...
def train_model(model, train_loader, val_loader, res_scaler, device):
    data_loss_fn = nn.SmoothL1Loss(beta=SMOOTH_L1_BETA)
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        train_losses = run_epoch(
            model, train_loader, data_loss_fn, res_mean, res_scale, device, optimizer
        )
        val_losses = run_epoch(
            model, val_loader, data_loss_fn, res_mean, res_scale, device
        )
        ...
        history.append({
            ...
            "train_data_loss": train_losses["data"],
            "train_physics_loss": train_losses["physics"],
            "val_total_loss": val_losses["total"],
            "val_data_loss": val_losses["data"],
            "val_physics_loss": val_losses["physics"],
            "lr": lr,
        })

        if val_losses["total"] < best_val - 1e-7:
            best_val, best_epoch, wait = val_losses["total"], epoch, 0
        ...
        if epoch == 1 or epoch % 20 == 0:
            print(
                f"Epoch {epoch:04d} | "
                f"train(total/data/phy)={train_losses['total']:.6f}/"
                f"{train_losses['data']:.6f}/{train_losses['physics']:.6f} | "
                f"val(total/data/phy)={val_losses['total']:.6f}/"
                f"{val_losses['data']:.6f}/{val_losses['physics']:.6f} | lr={lr:.2e}"
            )
        if wait >= PATIENCE:
...
def build_summary(df, train_idx, val_idx, test_idx, split_info, history, best_epoch, best_val, metrics):
    return {
        ...
        "training": {
            ...
            "loss_definition": "SmoothL1(scaled residual) + lambda_phy * physical penalty(raw final efficiency)",
        },
        "model_config": {
            "D_MODEL": D_MODEL,
            "N_HEADS": N_HEADS,
...
def plot_loss(history):
    ...
    ax.plot(h["epoch"], h["train_total_loss"], label="Train total loss")
    ax.plot(h["epoch"], h["val_total_loss"], label="Validation total loss")
    ax.set_xlabel("Epoch")
...
def main():
    ...
    history, best_epoch, best_val = train_model(
        model, train_loader, val_loader, scalers["res"], device
    )
    ...
    metrics = {
        ...
        "NN_raw_physics_constraints": physics_constraint_stats(test["pred_eff_raw"]),
    }
    ...
    print(
        "裁剪前预测的物理约束违反率: "
        f"{metrics['NN_raw_physics_constraints']['any_violation_rate_percent']:.2f}%"
    )
    print("=" * 80)
