...
OUTPUT_DIR = ROOT / "proposed_method_results"
...
# L = L_SmoothL1 + PHYSICS_LOSS_WEIGHT * L_phy
# 与第一份代码保持一致：采用 0.1 作为较弱的物理约束权重。
...
class ModalResidualNet(nn.Module):
    """
    Clean modal-token residual network without CLS token.

    Network input tokens:
        [Geometry, Mode0, Mode1]

    Token aggregation:
        Mean pooling over the three Transformer output tokens.
    ...
    def __init__(self):
        ...
        # 3 token types: Geometry, Mode0, Mode1
        ...
        # Lightweight MLP regression head: mean pooled feature(96) -> 64 -> 2.
    ...
    def forward(self, mode, geom, return_attention: bool = False):
        b = mode.shape[0]

        # Only three physical tokens are retained:
        # [Geometry, Mode0, Mode1]
        tokens = torch.cat([
        ...
        type_ids = torch.arange(3, device=tokens.device).unsqueeze(0).expand(b, -1)
        z = self.input_dropout(tokens + self.type_embedding(type_ids))

        attn_list = []
        for block in self.encoder:
            if return_attention:
                z, attn_weights = block(z, return_attention=True)
                attn_list.append(attn_weights)
            else:
                z = block(z, return_attention=False)

        # Mean pooling instead of CLS aggregation.
        pooled = z.mean(dim=1)
        pred = self.head(pooled)

        if return_attention:
            return pred, attn_list
        return pred
...
def inverse_scale_residual_torch(pred_res_scaled, res_mean, res_scale):
    """使用训练集 StandardScaler 参数，以可微方式还原网络预测的物理残差。"""
    return pred_res_scaled * res_scale + res_mean


def physics_loss_from_efficiency(pred_eff_raw):
    """
    对裁剪前的最终预测效率施加基本物理约束：
        eta_0 >= 0, eta_-1 >= 0, eta_0 + eta_-1 <= 1.
    """
    negative_eta0 = torch.relu(-pred_eff_raw[:, 0]).square()
    negative_etam1 = torch.relu(-pred_eff_raw[:, 1]).square()
    energy_excess = torch.relu(pred_eff_raw.sum(dim=1) - 1.0).square()
    return (negative_eta0 + negative_etam1 + energy_excess).mean()


def run_epoch(model, loader, loss_fn, res_mean, res_scale, device, optimizer=None):
    is_train = optimizer is not None
    model.train(is_train)
    total = 0.0

    for batch in loader:
        mode, geom, target, smm, *_ = unpack(batch, device)
        with torch.set_grad_enabled(is_train):
            pred = model(mode, geom)

            # 原有数据损失：标准化残差空间中的 SmoothL1。
            data_loss = loss_fn(pred, target)

            # 新增物理约束：作用于裁剪前最终效率 eta_SMM + residual_NN。
            pred_res = inverse_scale_residual_torch(pred, res_mean, res_scale)
            pred_eff_raw = smm + pred_res
            physics_loss = physics_loss_from_efficiency(pred_eff_raw)
...
def train_model(model, train_loader, val_loader, res_scaler, device):
    loss_fn = nn.SmoothL1Loss(beta=SMOOTH_L1_BETA)
    res_mean = torch.as_tensor(res_scaler.mean_, dtype=torch.float32, device=device)
    res_scale = torch.as_tensor(res_scaler.scale_, dtype=torch.float32, device=device)
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        train_loss = run_epoch(model, train_loader, loss_fn, res_mean, res_scale, device, optimizer)
        val_loss = run_epoch(model, val_loader, loss_fn, res_mean, res_scale, device)
...
def build_summary(df, train_idx, val_idx, test_idx, split_info, history, best_epoch, best_val, metrics):
    return {
        ...
        "model_config": {
            ...
            "USE_CLS_TOKEN": False,
            "TOKEN_AGGREGATION": "mean_pooling",
            "NUM_INPUT_TOKENS": 3,
            "USE_DELTA_AS_NETWORK_INPUT": False,
            "USE_SMM_EFFICIENCY_AS_NETWORK_INPUT": False,
        },
...
def collect_average_attention(model, loader, device):
    """
    Average attention over all test samples.

    Returns
    -------
    layer_attention : list of ndarray
        One [3, 3] matrix per Transformer layer, averaged over
        test samples and attention heads.

    overall_attention : ndarray
        [3, 3] matrix averaged over all Transformer layers.
    """
    model.eval()
    ...
    with torch.no_grad():
        for batch in loader:
            mode, geom, *_ = unpack(batch, device)

            _, attn_list = model(mode, geom, return_attention=True)
            batch_size = mode.shape[0]
            sample_count += batch_size

            for layer_idx, attn_weights in enumerate(attn_list):
                # attn_weights: [batch, n_heads, 3, 3]
                # First average over heads, then sum over batch.
                attn_mean_heads = attn_weights.mean(dim=1)
                layer_sum[layer_idx] += (
                    attn_mean_heads.sum(dim=0).cpu().numpy()
                )

    if sample_count == 0:
        raise RuntimeError("Attention analysis failed: empty loader.")

    layer_attention = [
        mat / sample_count
        for mat in layer_sum
    ]
    overall_attention = np.mean(
        np.stack(layer_attention, axis=0),
        axis=0
    )

    return layer_attention, overall_attention


def _draw_attention_matrix(ax, matrix, title):
    token_names = ["Geom", "Mode0", "Mode1"]
...
def plot_attention_maps(model, test_loader, device):
    """
    Save attention figures and CSV files.

    The attention matrix follows:
        row    = Query token
        column = Key token
    ...
    Token order:
        [Geom, Mode0, Mode1]
    """
    layer_attention, overall_attention = collect_average_attention(
    ...
    # --------------------------------------------------------
    # Figure 1: attention map for each Transformer layer
    # --------------------------------------------------------
    ...
    for layer_idx, matrix in enumerate(layer_attention):
        im = _draw_attention_matrix(
            axes[layer_idx],
            matrix,
            f"Layer {layer_idx + 1} Average Attention"
        )
    ...
    # Tighter spacing between the two heatmaps, and reserve a
    # dedicated colorbar axis on the far right to avoid overlap.
    fig.subplots_adjust(
        left=0.07,
        right=0.88,
        bottom=0.20,
    ...
    cbar_ax = fig.add_axes([0.895, 0.22, 0.018, 0.63])
    cbar = fig.colorbar(
        im,
        cax=cbar_ax
    )
    ...
    save_paper_fig(
        fig,
        "attention_per_layer_mean_pooling_10000"
    )

    # --------------------------------------------------------
    # Figure 2: overall average attention map
    # --------------------------------------------------------
    fig, ax = plt.subplots(figsize=(6.2, 5.35))
    im = _draw_attention_matrix(
        ax,
        overall_attention,
    ...
    save_paper_fig(
        fig,
        "attention_overall_mean_pooling_10000"
    )

    # --------------------------------------------------------
    # Save exact matrices as CSV for later paper analysis.
    # --------------------------------------------------------
    for layer_idx, matrix in enumerate(layer_attention):
        pd.DataFrame(
            matrix,
            ...
            OUTPUT_DIR / f"attention_layer{layer_idx + 1}_mean_pooling_10000.csv"
        )
    ...
    pd.DataFrame(
        ...
        OUTPUT_DIR / "attention_overall_mean_pooling_10000.csv"
    )
...
def main():
    ...
    print("=" * 80)
    print("SMM-guided Modal Token Residual Network - Mean Pooling - 10000 samples")
    print("Device:", device)
    ...
    history, best_epoch, best_val = train_model(model, train_loader, val_loader, scalers["res"], device)
    ...
    # Additional interpretability output:
    # attention among Geom / Mode0 / Mode1 after removing CLS.
    plot_attention_maps(model, test_loader, device)
    ...
    print("  8. figures/attention_per_layer_mean_pooling_10000.png 和 .svg")
    print("  9. figures/attention_overall_mean_pooling_10000.png 和 .svg")
    print(" 10. attention_layer*_mean_pooling_10000.csv")
    print(" 11. attention_overall_mean_pooling_10000.csv")
